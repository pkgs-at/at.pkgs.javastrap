...
(function(root, $, pkgs, unasigned) {
	...
	at.fn.number = function() {
		var format;
		var hankaku;

		format = arguments;
		hankaku = Object.has('', 'hankaku');
		this.$.each(function() {
			var $input;
			...
			$input.change(function() {
				...
				value = (hankaku ? text.hankaku() : text).toNumber();
				$input.val(!isNaN(value) ? Number.prototype.format.apply(value, format) : '');
			});
		});
		return this;
	};
	at.fn.date = function() {
		...
		this.$.each(function() {
			...
			$input.change(function() {
				...
				text = $input.val();
				if (text.length <= 0) return;
				value = Date.create(hankaku ? text.hankaku() : text);
				$input.val(value.isValid() ? Date.prototype.format.apply(value, format) : '');
			});
		});
		return this;
	};
